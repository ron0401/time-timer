...
test('iPhone navigation, twenty-minute limit, and stationary apples', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '20分', exact: true })).toHaveAttribute('aria-pressed', 'true');
  await expect(page.getByRole('button', { name: 'お知らせの音' })).toHaveCount(0);
  await page.getByRole('button', { name: '開始', exact: true }).click();
  await expect(page.locator('.apple-slot')).toHaveCount(20);
  ...
  await expect(page.locator('.is-eaten')).toHaveCount(19);
  await expect(page.getByRole('timer')).toHaveText('01:00');
  ...
  await page.clock.fastForward(60_000);
  await expect(page.locator('.is-eaten')).toHaveCount(20);
  await expect(page.getByRole('timer')).toHaveText('00:00');
  await expect(page.locator('#status')).toHaveText('終了');
  await page.goBack();
  await expect(page.locator('#timer-screen')).toBeHidden();
...
test('all twenty apples fit on one screen across iPhone sizes and rotation', async ({ page }) => {
  await page.goto('./');
  ...
  for (const [name, width, height] of [['desktop', 1440, 1000], ['iphone', 390, 844], ['small-iphone', 320, 568], ['small-iphone-browser', 320, 460], ['landscape', 844, 390]] as const) {
    ...
    await page.screenshot({ path: `test-results/${name}-twenty-apples.png`, fullPage: true, animations: 'disabled' });
