...
1. 1〜20分で時間を設定します。1・5・10・20分のボタンも使えます。
2. 「開始」でリンゴの画面に移ります。ヘッダーを隠して表示領域を広げ、画面に合う配置で最大20個をスクロールせずに表示します。1分かけて青虫がリンゴを1つ食べます。
3. 残り時間の横のボタンで一時停止・再開できます。全部食べたら終了です。
4. 「時間を変更」またはブラウザの戻る操作でタイマーを終了し、時間設定の画面に戻ります。設定した分数はそのまま残ります。
5. 音は鳴りません。終了は画面に表示されます。

iPhone・パソコンに対応。タブが裏に回っている間も経過時間を反映します。ページを再読み込みするとタイマーはリセットされます。
